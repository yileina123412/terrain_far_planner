...
enum AngleNoiseDirect { NARROW = 1, WIDE = -1, NO_NOISE = 0 };
...
public:
    ...
    static float kLocalPlanRange;  // 5m
    ...
    */
    static void ExtractNewObsPointCloud(const PointCloudPtr& cloudIn,
        const PointCloudPtr& cloudRefer, const PointCloudPtr& cloudNew);
    ...
    static void StackCloudByTime(
        const PointCloudPtr& curInCloud, const PointCloudPtr& StackCloud, const float& duration);

    static void TransferCloud(const Point3D& transPoint, const PointCloudPtr& cloudInOut);

    static void ConvertCloudToPCL(
        const PointStack& point_stack, const PointCloudPtr& point_cloud_ptr);

    static void ConvertCTNodeStackToPCL(
    ...
    static void TransformPCLFrame(const std::string& from_frame_id, const std::string& to_frame_id,
    ...
    static std::size_t PointInXCounter(
    ...
    static void Flat3DPointCloud(const PointCloudPtr& cloudIn, const PointCloudPtr& cloudFlat,
    ...
    static void ClearKdTree(const PointCloudPtr& cloud_ptr, const PointKdTreePtr& kdTree_ptr);
    ...
    static void EraseNodeFromStack(const NavNodePtr& node_ptr, NodePtrStack& node_stack);

    static float NoiseCosValue(
        const float& dot_value, const bool& is_large, const float& noise = FARUtil::kAngleNoise);

    static void CopyGroundPCL(const PointCloudPtr& freeCloudIn, const PointCloudPtr& freeCloudOut);

    static bool IsOutReducedDirs(const Point3D& diff_p, const PointPair& surf_dirs);

    static bool IsOutReducedDirs(const Point3D& diff_p, const NavNodePtr& node_ptr);

    static bool IsInCoverageDirPairs(const Point3D& diff_p, const NavNodePtr& node_ptr);

    static bool IsInContourDirPairs(const Point3D& diff_p, const PointPair& surf_dirs);

    static float MarginAngleNoise(
        const float& dist, const float& max_dist, const float& angle_noise);
    ...
    static bool IsDirsConverage(const PointPair& cur_directs, const PointPair& update_directs);

    static void InflateCloud(const PointCloudPtr& obsCloudInOut, const float& resol,
    ...
    static void ExtractOverlapCloud(const PointCloudPtr& cloudIn, const PointCloudPtr& cloudRef,
        const PointCloudPtr& cloudOverlapOut, const float& margin_ratio = 1.0);

    static void RemoveOverlapCloud(const PointCloudPtr& cloudInOut, const PointCloudPtr& cloudRef,
        const bool& is_copy_cloud = false);

    static void RemoveIndicesFromCloud(
    ...
    static inline bool IsAtSameLayer(
        const NodeType1& node_ptr1, const NodeType2& node_ptr2, const bool& is_large = false) {
        if (FARUtil::IsMultiLayer &&
            abs(node_ptr1->position.z - node_ptr2->position.z) > FARUtil::kTolerZ) {
            return false;
        }
        return true;
    }
    ...
    static void CreatePointsAroundCenter(const Point3D& center_p, const float& radius,
        const float& resol, PointStack& points_stack, const bool& is_sort = true);

    static Point3D RANSACPoisiton(
        const std::deque<Point3D>& pos_filter_stack, const float& margin, std::size_t& inlier_size);

    static PointPair RANSACSurfDirs(const std::deque<PointPair>& surf_dirs_stack,
        const float& margin, std::size_t& inlier_size);

    static float VerticalDistToLine2D(
        const Point3D& start_p, const Point3D& end_p, const Point3D& cur_p);

    static bool IsInCylinder(const Point3D& start_p, const Point3D& end_p, const Point3D& cur_p,
        const float& radius, const bool& is_2D = false);

    static float LineMatchPercentage(const PointPair& line1, const PointPair& line2);

    static Point3D ContourSurfDirs(
    ...
    static bool IsNodeInExtendMatchRange(const NavNodePtr& node_ptr) {
        if ((node_ptr->position - FARUtil::odom_pos).norm() < FARUtil::kSensorRange) {
            return true;
        }
        return false;
    }
    // 点是不是is_odom或is_navpoint
    ...
    static Point NormalizePXY(const Point& p) {
        ...
        if (norm > FARUtil::kEpsilon) {
            ...
            if (FARUtil::IsDebug)
                ROS_WARN("FARUtil: Point XY normalization fails, vector norm is too small.");
        }
        return return_p;
    }
    ...
    static bool PointInsideAPoly(const std::vector<Point>& poly, const Point& p) {
        ...
        if (npol < 3) {
            if (FARUtil::IsDebug)
                ROS_WARN("FARUtil: The vertices number of a polygon is less than 3.");
            return false;
        }
