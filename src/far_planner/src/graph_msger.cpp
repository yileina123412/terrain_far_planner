    ...
    for (std::size_t i = 0; i < graph_msg.nodes.size(); i++) {
        ...
        for (const auto& cid : connect_idxs) {
            ...
            if (cnode_ptr != NULL && (!node_ptr->is_active || !cnode_ptr->is_active ||
                                         (node_ptr->is_boundary && cnode_ptr->is_boundary))) {
...
void GraphMsger::CreateDecodedNavNode(
    const visibility_graph_msg::Node& vnode, NavNodePtr& node_ptr) {
    ...
    const PointPair surf_pair = {
        Point3D(vnode.surface_dirs[0].x, vnode.surface_dirs[0].y, vnode.surface_dirs[0].z),
...
void GraphMsger::ExtractConnectIdxs(const visibility_graph_msg::Node& node, IdxStack& connect_idxs,
    IdxStack& poly_idxs, IdxStack& contour_idxs, IdxStack& traj_idxs) {
