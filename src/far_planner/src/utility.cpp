...
// void FARUtil::ExtractNewObsPointCloud(const PointCloudPtr& cloudIn,  // 当前帧点云
//     const PointCloudPtr& cloudRefer,                                 // 历史参考点云
//     const PointCloudPtr& cloudNew) {                                 // 输出：cloudIn多出的点
...
//     kdtree.setInputCloud(cloudRefer);

//     // 搜索半径（根据体素尺寸）
//     const float search_radius = FARUtil::kLeafSize * 3.0f;
...
//         // 在历史点云中搜索半径范围内的点
//         int found = kdtree.radiusSearch(p, search_radius, indices, distances, 1);
...
//     }
// }
void FARUtil::ExtractNewObsPointCloud(
    const PointCloudPtr& cloudIn, const PointCloudPtr& cloudRefer, const PointCloudPtr& cloudNew) {
    cloudNew->clear();
    if (cloudRefer->empty()) {
        *cloudNew = *cloudIn;
        return;
    ...
    for (const auto& p : cloudIn->points) {
        ...
        if (kdtree.nearestKSearch(p, 1, indices, distances) > 0) {
            const float nearest_dist = sqrt(distances[0]);

            // 【关键】动态阈值：根据距离机器人的远近判断
            const float dist_to_robot = (Point3D(p) - FARUtil::robot_pos).norm();
            float dist_threshold = 0.5;

            // if (dist_to_robot < 5.0f) {
            //     dist_threshold = FARUtil::kLeafSize * 3.0f;
            // } else if (dist_to_robot < 15.0f) {
            //     dist_threshold = FARUtil::kLeafSize * 5.0f;
            // } else {
            //     dist_threshold = FARUtil::kLeafSize * 8.0f;
            // }
...
void FARUtil::CropPCLCloud(const PointCloudPtr& cloudIn, const PointCloudPtr& cloudCropped,
    const Point3D& centriod, const float& range) {
...
void FARUtil::CropPCLCloud(
    const PointCloudPtr& cloudInOut, const Point3D& centriod, const float& range) {
...
void FARUtil::TransformPCLFrame(const std::string& from_frame_id, const std::string& to_frame_id,
    const tf::TransformListener* tf_listener, const PointCloudPtr& cloudInOut) {
...
void FARUtil::TransformPoint3DFrame(const std::string& from_frame_id,
    const std::string& to_frame_id, const tf::TransformListener* tf_listener, Point3D& point) {
...
void FARUtil::ConvertCloudToPCL(
    const PointStack& point_stack, const PointCloudPtr& point_cloud_ptr) {
...
void FARUtil::ExtractFreeAndObsCloud(const PointCloudPtr& newCloudIn,
    const PointCloudPtr& freeCloudOut, const PointCloudPtr& obsCloudOut) {
    // pre-process cloud
...
void FARUtil::ClearKdTree(const PointCloudPtr& cloud_ptr, const PointKdTreePtr& kdTree_ptr) {
    PCLPoint temp_p;
...
bool FARUtil::IsPointNearNewPoints(const Point3D& p, const bool& is_creation) {
    ...
    const int counter_limit =
        is_creation ? std::round(FARUtil::KNewPointC / 2.0f) : FARUtil::KNewPointC;
    return (near_c > counter_limit) ? true : false;
}

std::size_t FARUtil::PointInXCounter(
    const Point3D& p, const float& radius, const PointKdTreePtr& KdTree) {
...
void FARUtil::Flat3DPointCloud(
    const PointCloudPtr& cloudIn, const PointCloudPtr& cloudFlat, const bool& is_downsample) {
    cloudFlat->clear();
...
void FARUtil::EraseNodeFromStack(const NavNodePtr& node_ptr, NodePtrStack& node_stack) {
...
void FARUtil::SetDifference(std::vector<int>& v1, std::vector<int>& v2, std::vector<int>& diff) {
    ...
    diff.clear();
    std::set_difference(
        v1.begin(), v1.end(), v2.begin(), v2.end(), std::inserter(diff, diff.begin()));
}

float FARUtil::MarginAngleNoise(
    const float& dist, const float& max_shift_dist, const float& angle_noise) {
...
bool FARUtil::IsOutReducedDirs(const Point3D& diff_p, const PointPair& surf_dirs) {
    const Point3D norm_dir = diff_p.normalize_flat();
    const float margin_angle_noise =
        FARUtil::MarginAngleNoise(diff_p.norm_flat(), FARUtil::kNearDist, FARUtil::kAngleNoise);
    Point3D temp_opposite_dir;
    // check first half range
    temp_opposite_dir = -surf_dirs.second;
    float in_res_thred =
        FARUtil::NoiseCosValue(surf_dirs.first * temp_opposite_dir, true, margin_angle_noise);
    if (norm_dir * surf_dirs.first > in_res_thred && norm_dir * temp_opposite_dir > in_res_thred) {
        return true;
    }
    // check second half range
    temp_opposite_dir = -surf_dirs.first;
    in_res_thred =
        FARUtil::NoiseCosValue(surf_dirs.second * temp_opposite_dir, true, margin_angle_noise);
    if (norm_dir * surf_dirs.second > in_res_thred && norm_dir * temp_opposite_dir > in_res_thred) {
        return true;
    }
    return false;
}

bool FARUtil::IsOutReducedDirs(const Point3D& diff_p, const NavNodePtr& node_ptr) {
...
bool FARUtil::IsInCoverageDirPairs(const Point3D& diff_p, const NavNodePtr& node_ptr) {
    ...
    const float margin_angle_noise = FARUtil::MarginAngleNoise(
        diff_p.norm_flat(), FARUtil::kNearDist, FARUtil::kAngleNoise * 2.0f);
    const float dot_value =
        FARUtil::NoiseCosValue(surf_dirs.first * surf_dirs.second, true, margin_angle_noise);
...
bool FARUtil::IsInContourDirPairs(const Point3D& diff_p, const PointPair& surf_dirs) {
    const float margin_angle_noise =
        FARUtil::MarginAngleNoise(diff_p.norm_flat(), FARUtil::kNearDist, FARUtil::kAngleNoise);
...
void FARUtil::InflateCloud(const PointCloudPtr& obsCloudInOut, const float& resol,
    const int& inflate_size, const bool& deep_z_inflate) {
    const std::size_t current_size = obsCloudInOut->size();
    const int z_size = deep_z_inflate ? inflate_size + 1 : inflate_size;
    const std::size_t array_size =
        current_size * (pow(inflate_size * 2 + 1, 2) * (z_size * 2 + 1) + 1);
...
void FARUtil::ClusterFilterCloud(
    const PointCloudPtr& cloudInOut, const float& radius, const std::size_t c_thred) {
    ...
    for (const auto& p : cloudInOut->points) {
        const std::size_t counter =
            FARUtil::PointInXCounter(Point3D(p), radius, FARUtil::kdtree_filter_cloud_);
        if (counter > c_thred) {
...
void FARUtil::ExtractOverlapCloud(const PointCloudPtr& cloudIn, const PointCloudPtr& cloudRef,
    const PointCloudPtr& cloudOverlapOut, const float& margin_ratio) {
...
void FARUtil::RemoveOverlapCloud(
    const PointCloudPtr& cloudInOut, const PointCloudPtr& cloudRef, const bool& is_copy_cloud) {
...
void FARUtil::StackCloudByTime(
    const PointCloudPtr& curInCloud, const PointCloudPtr& StackCloud, const float& duration) {
...
void FARUtil::RemoveIndicesFromCloud(
    const pcl::PointIndices::Ptr& outliers, const PointCloudPtr& cloudInOut) {
...
float FARUtil::VerticalDistToLine2D(
    const Point3D& start_p, const Point3D& end_p, const Point3D& cur_p) {
...
Point3D FARUtil::ContourSurfDirs(
    const Point3D& end_p, const Point3D& start_p, const Point3D& center_p, const float& radius) {
...
Point3D FARUtil::RANSACPoisiton(
    const std::deque<Point3D>& pos_filter_stack, const float& margin, std::size_t& inlier_size) {
    inlier_size = 0;
...
PointPair FARUtil::RANSACSurfDirs(
    const std::deque<PointPair>& surf_dirs_stack, const float& margin, std::size_t& inlier_size) {
    inlier_size = 0;
...
void FARUtil::ConvertCTNodeStackToPCL(
    const CTNodeStack& ctnode_stack, const PointCloudPtr& point_cloud_ptr) {
...
void FARUtil::CropBoxCloud(
    const PointCloudPtr& cloudInOut, const Point3D& center_p, const Point3D& crop_size) {
    pcl::CropBox<PCLPoint> boxFilter;
    pcl::PointCloud<PCLPoint> cropBox_cloud;
    Eigen::Vector4f min_vec(
        center_p.x - crop_size.x, center_p.y - crop_size.y, center_p.z - crop_size.z, 1.0f);
...
bool FARUtil::IsInCylinder(const Point3D& from_p, const Point3D& end_p, const Point3D& cur_p,
    ...
    const Point3D unit_axial =
        is_2D ? (end_p - from_p).normalize_flat() : (end_p - from_p).normalize();
...
void FARUtil::CreatePointsAroundCenter(const Point3D& center_p, const float& radius,
    const float& resol, PointStack& points_stack, const bool& is_sort) {
...
bool FARUtil::IsPointInLocalRange(const Point3D& p, const bool& is_large_h) {
    ...
    if (FARUtil::IsPointInToleratedHeight(p, H) &&
        (p - FARUtil::odom_pos).norm() < FARUtil::kSensorRange) {
        return true;
    }
    return false;
}
// 判断是否在局部感知边界： 30m-0.8m
bool FARUtil::IsPointInMarginRange(const Point3D& p) {
    if (FARUtil::IsPointInToleratedHeight(p, FARUtil::kMarginHeight) &&
        (p - FARUtil::odom_pos).norm() < FARUtil::kMarginDist) {
        return true;
    }
    return false;
}
...
bool FARUtil::ClockwiseLess(const Point3D& a, const Point3D& b) {
    ...
    if (a.x >= 0.0f && b.x < 0.0f) return true;
    if (a.x < 0.0f && b.x >= 0.0f) return false;
...
void FARUtil::ClockwiseTwoPoints(const Point3D& center, PointPair& edge) {
    ...
    PointStack vertices;
    edge_copy.first = edge_copy.first - c;
    vertices.push_back(edge_copy.first);
    edge_copy.second = edge_copy.second - c;
    vertices.push_back(edge_copy.second);
    center_copy = center_copy - c;
    vertices.push_back(center_copy);
...
float FARUtil::LineMatchPercentage(const PointPair& line1, const PointPair& line2) {
    ...
    const float theta =
        acos((line1.second - line1.first).norm_flat_dot(line2.second - line2.first));
