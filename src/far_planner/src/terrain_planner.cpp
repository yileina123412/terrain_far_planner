 ...
 * 局部地形路径规划器，用于在机器人附近的小范围内（通常5-10米），基于网格地图和A*算法，规划从一个导航点到另一个导航点的无碰撞路径。
 */
...
void TerrainPlanner::Init(const ros::NodeHandle& nh, const TerrainPlannerParams& params) {
    ...
    tp_params_ = params;
    row_num_ =
        std::ceil((FARUtil::kLocalPlanRange + tp_params_.radius) * 2.0f / tp_params_.voxel_size);
    col_num_ = row_num_;
    TerrainNodePtr init_terrain_node_ptr = NULL;
    Eigen::Vector3i grid_size(row_num_, col_num_, 1);
    Eigen::Vector3d grid_origin(0, 0, 0);
    Eigen::Vector3d grid_resolution(
        tp_params_.voxel_size, tp_params_.voxel_size, tp_params_.voxel_size);
    terrain_grids_ = std::make_unique<grid_ns::Grid<TerrainNodePtr>>(
        grid_size, init_terrain_node_ptr, grid_origin, grid_resolution, 3);
...
void TerrainPlanner::SetLocalTerrainObsCloud(const PointCloudPtr& obsCloudIn) {
    ...
    for (const auto& point : obsCloudIn->points) {
        Eigen::Vector3i c_sub =
            terrain_grids_->Pos2Sub(Eigen::Vector3d(point.x, point.y, center_pos_.z));
        // 对每个障碍物点云，向周围膨胀 N_IF 个格子
...
// A*路径规划
bool TerrainPlanner::PlanPathFromPToP(
    const Point3D& from_p, const Point3D& to_p, PointStack& path) {
    // 在圆柱形搜索范围内，用A*算法规划路径
    // 代价函数：欧氏距离
    // 搜索空间：8邻域（上下左右+对角线）
    path.clear();
    ...
    if (!terrain_grids_->InRange(start_sub) || !terrain_grids_->InRange(end_sub)) {
        if (FARUtil::IsDebug)
            ROS_WARN("TP: two interval navigation nodes are not in terrain planning range.");
        return false;
    }
    const TerrainNodePtr start_node_ptr =
        terrain_grids_->GetCell(terrain_grids_->Sub2Ind(start_sub));
    ...
    while (true) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (close_set.count(neighbor->id) ||
                (neighbor->is_occupied && neighbor != end_node_ptr) || !InCylinder(neighbor))
                continue;
